...
class SpiderAlfacasaSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.tayara.tn/fr/search/?category=Immobilier"]
    ...
    def parse(self, response):
      ...
      #  nbpieces=[]
       ...
       # links=[]
        ...
        details=[]
        for i in range (1,10):
            PRIX = response.css('mr-1').getall()
            TYPE = response.css('truncate text-3xs md:text-xs lg:text-xs w-3/5 font-medium text-neutral-500').getall()
            ANNONCE_DATE = response.css('line-clamp-1 truncate text-3xs md:text-xs lg:text-xs w-3/5 font-medium text-neutral-500').getall()
           ...
           # link =response.css('div.read-more-wrapper a::attr(href)').getall()
            ...
            for e in dateannonce:
                dateannonce = e.strip()
                ANNONCE_DATE.append(dateannonce)
          ...
          #  insert_stmt =insert_stmt + "%s,%s,%s,%s,%s,%s,%s,%s)"
            ...
            insert_stmt = "INSERT INTO scrapingr_annonce_copy (prix, type, dateannonce) VALUES ("
            insert_stmt =insert_stmt + "%s,%s,%s)"
            
           
            
        for i in range(1,10):    
            scrapped_data = (PRIX[i],TYPE[i],ANNONCE_DATE[i])
